...
\DeclareAcronym{OU}{
  short = {ОУ},
  long = {операционный усилитель},
	unit = --, 
}

\DeclareAcronym{IS}{
  short = {ИС},
  long = {интегральная схема},
	unit = --, 
}
...
\DeclareAcronym{SPI}{
  short = SPI,
  long = Serial Periphial Interface ,
  unit = --, 
}
\DeclareAcronym{mk}{
  short = МК,
  long  = микроконтроллер ,
  unit = --, 
}
...
\begin{document}
\input{01_title}% это титульный лист
\input{02_task}% это задание
\input{03_referat}% это реферат
\input{04_cont}% это содержание
\input{05_intro}% это введение
\input{06_structure}% это раздел структурной схемы
\input{07_princip}% это раздел принципиальной схемы
\input{08_program}% это раздел принципиальной схемы
\input{09_final}% это заключение
\input{10_biblio}% это список использованных источников
\input{11_socr}% это список cокращений и источников
...
{main.c}%файл с кодом(должен лежать в папке проекта)
...
{C}%язык программирования
{ads1293.c}%файл с кодом(должен лежать в папке проекта)
...
{C}%язык программирования
{ads1293.h}%файл с кодом(должен лежать в папке проекта)
